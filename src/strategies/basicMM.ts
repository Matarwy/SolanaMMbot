...
export class MarketMaker {
    ...
    constructor() {
        ...
        this.mcbToken = { address: MBC_MINT_ADDRESS, symbol: 'BNOM', decimals: 6 };
        ...
        this.slippageBps = 200; // 0.5%
        this.priceTolerance = 0.005; // .5%
        this.rebalancePercentage = 0.75; // 75%
    }
        ...
        while (true) {
            for (const pair of tradePairs) {
                try {
                    ...
                    console.error(`Error during trade evaluation/execution for pair ${pair.token0}-${pair.token1}:`, error);
                }
            }
    ...
    async evaluateAndExecuteTrade(jupiterClient: JupiterClient, pair: any, enableTrading: Boolean): Promise<void> {
        ...
        const token1Balance = await this.fetchTokenBalance(jupiterClient, pair.token1); // BNOM balance
        ...
        if (tradeNeeded) {
            ...
            if (solAmountToTrade.gt(0)) {
                console.log(`Trading ${solAmountToTrade.toString()} SOL for BNOM...`);
                ...
                console.log(`Trading ${mbcAmountToTrade.toString()} BNOM for SOL...`);
